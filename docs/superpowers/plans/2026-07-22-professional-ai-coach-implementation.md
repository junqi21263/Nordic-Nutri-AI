...
The implementation uses a separate idempotent migration, `0010_coach_permissions.sql`, to create missing legacy coach tables and enforce server-only access. In environments that already applied the core schema, it only verifies the existing tables/index and applies the access controls.
