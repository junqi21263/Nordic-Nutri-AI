...
**Architecture:** 保留 `/coach-answer` JSON 接口。新增认证后的 `/coach-answer/stream`：规则答案直接完成；合规饮食问题收集 DeepSeek SSE、验证完整结构化回答、持久化后输出 NDJSON 显示文本分块。小程序优先消费 NDJSON，读取失败时回退现有接口。NOVA 是本地组件，复用在教练消息与思考状态。
...
- `mini-program/src/styles/page.scss`：头像和流气泡样式；不修改固定底部布局。
...
- [ ] **Step 1: 写失败测试。** 添加 `streams a validated nutrition reply and persists only after completion`：注入 `streamAnswer: async function* () { yield validReplyJson; }`，调用 `collect(service.streamMessage("user-1", validRequest))`，断言事件类型是 `["delta", "complete"]`，并且 `coach_messages` 恰有一条 assistant 行。另添加非营养输入断言：没有调用 `streamAnswer`，只返回一条 `complete` 且 provider 是 `rule_v2`。

- [ ] **Step 2: 确认失败。** 运行 `node --test coach-data-service.test.mjs`；预期 `streamMessage is not a function`。

- [ ] **Step 3: 最小实现。** 扩展工厂参数为 `streamAnswer`，并新增 `streamMessage(userId, input)`。先 `normalizeInput`、`getPriorReply`、`buildContext`、`createRuleReply`；幂等键已有结果时只 yield `complete`。医疗风险、非营养输入和无流服务时，调用共享 `persistReply(userId, request, context, reply, source)` 并 yield `complete`。合规输入中收集所有 `streamAnswer` chunk 为 raw JSON，调用既有 `validateReply(raw)`；验证通过后 `persistReply`，再以最多 32 字符的 `splitForDisplay(content)` 依次 yield `{type:"delta",text}`，最后 yield `{type:"complete",...saved}`。任何上游或校验异常都持久化 `rule_v2` 完整答案，不能留下未配对的用户消息。`sendMessage` 同样复用 `persistReply`。
...
### Task 4: 建立 NOVA 原创头像组件

**Files:** Create `mini-program/src/components/coach-avatar/index.tsx` and `mini-program/src/components/coach-avatar/index.scss`; modify `mini-program/tests/coach-profile.test.ts`.
...
- [ ] **Step 3: 最小实现。** 新建 `CoachAvatar({status = "idle"})`，根节点 class 为 `coach-avatar coach-avatar--${status}`、`ariaLabel="NOVA 营养教练"`，内部由发型、脸部、深绿针织上衣和思考点组成。SCSS 使用圆形 40px、暖米白背景、深绿自然卷、温和微笑和仅在 `thinking` 时的三点呼吸动画；不得使用外链、生成图片或新增固定元素。
...
### Task 5: 接入小程序 NDJSON 消费与普通 JSON 回退
