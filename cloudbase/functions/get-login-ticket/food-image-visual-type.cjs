...
const RULES = Object.freeze([
  ...
  ["beverage_liquid", 91, ["energy drink", "sports drink", "coffee drink", "tea drink", "soy milk", "oat milk", "果汁", "牛奶", "豆奶", "燕麦奶", "汽水", "可乐", "苏打水", "茶饮", "咖啡饮料", "能量饮料", "运动饮料", "juice", "milk", "soda", "cola"]],
  ...
  ["tea_leaf", 63, ["茶叶", "绿茶", "红茶", "tea leaf", "tea leaves"]], ["oil_liquid", 62, ["橄榄油", "菜籽油", "芝麻油", "食用油", "oil"]], ["grain", 61, ["燕麦", "藜麦", "荞麦", "谷物", "grain", "oats", "quinoa"]],
...
function findRule(text) {
  ...
  for (let order = 0; order < RULES.length; order += 1) {
    ...
    for (const keyword of keywords) if (evidence.includes(clean(keyword))) candidates.push({ visualType, keyword, priority, order });
  }
  candidates.sort((a, b) => b.keyword.length - a.keyword.length || b.priority - a.priority || a.order - b.order);
  if (!candidates.length) return null;
  const winner = candidates[0];
  return { visualType: winner.visualType, matchedKeywords: [...new Set(candidates.filter((item) => item.visualType === winner.visualType).map((item) => item.keyword))] };
}

function resolveFoodVisualType(food = {}) {
  const manual = foodValue(food, "visualType", "visual_type");
  if (hasVisualType(manual) && manual !== "unknown") return { visualType: manual, matchedKeywords: [manual], source: "manual" };
  ...
  for (const tag of Array.isArray(food.tags) ? food.tags : []) {
    ...
    if (hasVisualType(code) && code !== "unknown") return { visualType: code, matchedKeywords: [code], source: "tags" };
  }
  const sources = [
    ["tags", tagText(food.tags)],
    ["nameZh", foodValue(food, "nameZh", "foodNameZh", "name_zh")],
    ["nameEn", foodValue(food, "nameEn", "foodNameEn", "name_en")],
    ["category", [food.category?.code, food.category?.nameZh, food.category?.nameEn, foodValue(food, "category", "categoryCode")].filter(Boolean).join(" ")],
  ];
  for (const [source, text] of sources) {
    const matched = findRule(text);
    if (matched) return { ...matched, source };
  }
  return { visualType: "unknown", matchedKeywords: [], source: "fallback" };
}
...
function resolveFlavorColor(food = {}) {
  ...
  const rules = [["orange", "浅橙黄色", ["橙味", "orange"]], ["lemon", "淡黄色", ["柠檬味", "lemon"]], ["strawberry", "淡粉色", ["草莓味", "strawberry"]], ["grape", "低饱和淡紫色", ["葡萄味", "grape"]], ["berry", "淡紫红色", ["莓果味", "berry"]], ["fruit", "浅黄色或浅橙色", ["水果味", "fruit flavored", "fruit-flavored"]], ["chocolate", "浅棕色", ["巧克力味", "chocolate"]], ["vanilla", "奶白色", ["香草味", "vanilla"]], ["matcha", "浅绿色", ["抹茶味", "matcha"]], ["coffee", "棕色", ["咖啡味", "coffee"]]];
...
function buildPromptByVisualType(food = {}, visualType = "unknown") {
  ...
  const [subject, typeNegative] = TYPE_DETAILS[type] || TYPE_DETAILS.unknown;
  ...
  return { templateName: `${FOOD_VISUAL_TYPE_OPTIONS[type]}模板`, subject: `主体：${nameZh}${nameEn ? `；英文：${nameEn}` : ""}。${subject}${color ? `。${color}` : ""}${antiAmbiguity ? `。${antiAmbiguity}` : ""}`, negativePrompt: [COMMON_NEGATIVE, typeNegative].filter(Boolean).join("、"), flavorColor };
}
...
module.exports = { FOOD_VISUAL_TYPES, FOOD_VISUAL_TYPE_OPTIONS, PROCESSING_LEVEL_BY_VISUAL_TYPE, resolveFoodVisualType, resolveFlavorColor, resolveFoodProcessingLevel, buildBasePhotographyPrompt, buildPromptByVisualType };
