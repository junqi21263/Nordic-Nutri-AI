    ...
    async load(userId: string) {
      ...
      dependencies.hydrate(
        ...
        },
        {
          ...
          foodAvoidances: Array.isArray(settings.food_avoidances)
            ? settings.food_avoidances.filter((item): item is string => typeof item === "string")
            : [],
          mealsPerDay:
            typeof settings.meals_per_day === "number" ? settings.meals_per_day : undefined,
          theme:
            settings.theme === "light" || settings.theme === "dark" || settings.theme === "system"
              ? settings.theme
              : undefined,
          language:
            settings.locale === "zh-CN" || settings.locale === "en" ? settings.locale : undefined,
          unit:
            settings.unit_system === "metric" || settings.unit_system === "imperial"
              ? settings.unit_system
              : undefined,
          notification:
            typeof settings.notification_enabled === "boolean"
